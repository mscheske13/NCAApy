    ...
    for n in [0, 2]:
        for event in pbp[f'{col_names[n + 1]}'].dropna():
            ...
            if 'substitution in' in event:
                ...
                if player not in starters[n] and len(starters[n]) < 5 and player not in benches[n]:
                    starters[n].append(player)
                if len(starters[n]) == 5:
                    break
            elif player not in starters[n] and player not in benches[n] and len(event.split(',')) > 1:
                starters[n].append(player)
                if len(starters[n]) == 5:
                    break
    starters.pop(1)
...
def to_lineup_df(pbp, starters, positions, is_home):
    ...
    to_add.columns = [f'{col_names[col]}_1', f'{col_names[col]}_2', f'{col_names[col]}_3',
                      f'{col_names[col]}_4', f'{col_names[col]}_5']
    return to_add
...
def event_packer(pbp):
    ...
    for index, time in enumerate(pbp['Time']):
        ...
        while (index + extra < len(pbp) and
               time == pbp['Time'][index + extra]):
...
def time_convert(time_str, half, total_halves):
    ...
    total_remaining_time = remaining_in_current_half + remaining_in_future_halves
